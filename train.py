...
import slam3r.utils.croco_misc as misc  # noqa
from slam3r.utils.croco_misc import NativeScalerWithGradNormCount as NativeScaler  # noqa


def get_args_parser():
    parser = argparse.ArgumentParser('SLAM3R training', add_help=False)
    # model and criterion
    parser.add_argument('--model', required=True,
                        type=str, help="string containing the model to build")
    parser.add_argument('--pretrained', default=None, help='path of a starting checkpoint')
    parser.add_argument('--pretrained_type', default='dust3r', help='type of pretrained checkpoint')
    parser.add_argument('--train_criterion', required=True,
    ...
    parser.add_argument('--ref_ids', nargs="+", type=int, help='the ids of reference views')
    ...
    parser.add_argument('--freeze', default=None, type=str, help='freeze which part of the model')
    ...
    parser.add_argument('--assist_model', type=str, default=None, help='the path of the assist model')
...
def main(args):
    ...
    #print info to txt
    if args.save_config and misc.is_main_process():
        config_file = open(os.path.join(args.output_dir, "console_log.txt"), mode="w", encoding="utf-8")
        sys.stdout = config_file
    ...
    if args.assist_model is not None:
        print('Loading assist model: {:s}'.format(args.assist_model))
        assist_model = eval(args.assist_model)
        assist_model.eval()
    ...
    if args.distributed:
        ...
        if args.assist_model is not None:
            assist_model = torch.nn.parallel.DistributedDataParallel(
                assist_model, device_ids=[args.gpu], find_unused_parameters=True, static_graph=True)

    if(hasattr(model_without_ddp, 'set_freeze') and args.freeze is not None):
        print("Freezing: ", args.freeze)
        model_without_ddp.set_freeze(args.freeze)
    ...
    def write_log_stats(epoch, train_stats, test_stats):
        if misc.is_main_process():
            ...
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                f.write(json.dumps(log_stats,indent='') + "\n")
    ...
    for epoch in range(args.start_epoch, args.epochs+1):
        ...
        if epoch > args.start_epoch and args.save_last:
        ...
        if (epoch > 0 and args.eval_freq > 0 and epoch % args.eval_freq == 0):
            ...
            for test_name, testset in data_loader_test.items():
                stats = test_one_epoch(model, assist_model,
                                       test_criterion, testset,
        ...
        train_stats = train_one_epoch(
            model, assist_model, 
            train_criterion, data_loader_train,
...
def build_dataset(dataset, batch_size, num_workers, test=False):
    ...
    loader = get_multi_data_loader(dataset,
...
def train_one_epoch(model: torch.nn.Module, assist_model: torch.nn.Module,
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        # print("batch: ", data_iter_step)
        epoch_f = epoch + data_iter_step / len(data_loader)  #"f" means float

        # we use a per iteration (instead of per epoch) lr scheduler
        if data_iter_step % accum_iter == 0:
            misc.adjust_learning_rate(optimizer, epoch_f, args)
        loss_tuple = loss_of_one_batch(args.loss_func , batch, model, criterion, 
                                       device, bool(args.amp), ret='loss', 
                                       assist_model=assist_model, 
                                       train=True, epoch=epoch_f,
                                       args=args)
        ...
        if not math.isfinite(loss_value):
            ...
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                f.write("proc{}, Epoch {}, iter {}, Loss {}, skip".format(misc.get_rank(),epoch, data_iter_step, loss_value))
            #清除梯度
            optimizer.zero_grad()
            del loss, loss_details, loss_value, loss_tuple
            del batch
            continue
...
@torch.no_grad()
def test_one_epoch(model: torch.nn.Module, assist_model: torch.nn.Module,
                   criterion: torch.nn.Module,
